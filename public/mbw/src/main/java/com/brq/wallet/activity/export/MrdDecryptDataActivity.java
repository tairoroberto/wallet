...
import com.brq.wallet.activity.TextNormalizer;
import com.brq.wallet.service.MrdKeyStretchingTask;
...
public class MrdDecryptDataActivity extends Activity implements TaskExecutionServiceController.TaskExecutionServiceCallback {
   ...
   class ProgressUpdater implements Runnable {
      ...
      public void run() {
         ...
         if (_taskStatus == null || _taskStatus.state == ServiceTaskStatusEx.State.NOTRUNNING) {
            ((TextView) findViewById(R.id.tvProgress)).setText("");
         } else if (_taskStatus.state == ServiceTaskStatusEx.State.RUNNING) {
   ...
   public void onStatusReceived(ServiceTaskStatusEx status) {
      _taskStatus = status;
      if (_taskStatus != null && _taskStatus.state == ServiceTaskStatusEx.State.FINISHED && _encryptionParameters == null) {
